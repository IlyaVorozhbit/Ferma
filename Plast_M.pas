...
type
 TPlast_Form = class(TForm)
    ...
    N4: TMenuItem;
    Razm: TLabel;
    s_text_mnu: TMenuItem;
    N6: TMenuItem;
    o_text_mnu: TMenuItem;
    N1: TMenuItem;
    Plast_ViewConstr_Mnu: TMenuItem;
    Plast_ViewGraph_Mnu: TMenuItem;
    N7: TMenuItem;
    Plast_Num_Element: TMenuItem;
    ...
    procedure Plast_ViewConstr_MnuClick(Sender: TObject);
    procedure Plast_ViewGraph_MnuClick(Sender: TObject);
    procedure Plast_Num_ElementClick(Sender: TObject);
...
//my insert
  s_text_mnu.Enabled:=false;
  simpleresults_mnu.Enabled:=false;
  main_form.SimpleRez_text.Enabled:=false;
  main_form.SimpleRez_graf.Enabled:=false;
  o_text_mnu.Enabled:=false;
  optresults_mnu.Enabled:=false;
  main_form.OptRez_text.Enabled:=false;
  main_form.OptRez_graf.Enabled:=false;
//end of my insert
  FormActivate(Self);
  isChanged:=true;
...
begin
 ...
 if plast.kl1<=0 then begin main_form.plast_Sn_cbx.Enabled:=false; end
 else  begin main_form.plast_Sn_cbx.Enabled:=true; end;
...
//my insert
  ...
  s_text_mnu.Enabled:=true;
  simpleresults_mnu.Enabled:=true;
  main_form.SimpleRez_text.Enabled:=true;
  main_form.SimpleRez_graf.Enabled:=true;
  end;
  ...
  o_text_mnu.Enabled:=true;
  optresults_mnu.Enabled:=true;
  main_form.OptRez_text.Enabled:=true;
  main_form.OptRez_graf.Enabled:=true;
  end;
//end of my insert
  FormActivate(Self);
  isChanged:=false;
...
begin
  ...
  Plast_ViewGraph_Mnu.Checked:= True;
  Main_Form.PlastGraphButton.Down := True;
...
//      visio_form.Dote_Rez_Btn.down:=true;
     ...
     Main_Form.Plast_Sn_Cbx.clear; Main_Form.Plast_Sn_Cbx.Items.Add('Случай нагружения 1');
     Main_Form.Plast_Sn_Cbx.ItemIndex:=0;
...
begin
 with PaintBox.Canvas do
  ...
  if Main_Form.Plast_Sn_Cbx.ItemIndex>=0 then Visio_form.num_force:=Main_Form.Plast_Sn_Cbx.ItemIndex+1;
  ii:=1;
...
{ if (Main_Form.Plast_Sn_Cbx.ItemIndex = -1)or( Main_Form.Plast_Sn_Cbx.Items.Count<>plast.kl1 ) then
  begin
   Main_Form.Plast_Sn_Cbx.clear;
   for i:=1 to plast.kl1 do
    begin
      Main_Form.Plast_Sn_Cbx.Items.Add('Случай нагружения '+IntToStr(i));
    end;
  end;

 if Main_Form.Plast_Sn_Cbx.ItemIndex = -1 then
  Main_Form.Plast_Sn_Cbx.ItemIndex:=0
 else
  Main_Form.Plast_Sn_Cbx.ItemIndex:=plast_fd_form.Nagr_Nagr_ComboBox.ItemIndex;}
//==============================================================================
        ...
        begin
          ...
          for i:=1 to n_of_step do
            begin
              ...
              MoveTo(x_max-30-round(mas_x*step*(i-1)),y_max-20-coord_axis_y);
              LineTo(x_max-30-round(mas_x*step*(i-1)),y_max-30-coord_axis_y);
            end;
        end
      else}
        begin
        ...
        begin
          ...
          for i:=1 to n_of_step do
            begin
              ...
              TextOut(3,23+round(mas_y*step*(i-1)),number);
              MoveTo(43+coord_axis_x,30+round(mas_y*step*(i-1)));
              LineTo(50+coord_axis_x,30+round(mas_y*step*(i-1)));
            end;
        end
      else }
        begin
...
//======== конец узелочкам ============
 ...
 with PaintBox.Canvas do
  begin
    if Main_Form.PlastNumberButton.Down then
     begin
      brush.color:=clWhite;
...
//  Main_Form.Save_TBtn.Enabled:=true; my comment
  Main_Form.Caption:='САПР Ferma 8.1 - подсистема '+#39+'Пластина'+#39;
...
//  Main_Form.TOK_ToolBar.Visible:=False; my comment
  ...
  Main_Form.Plast_Panel.Visible:=True;
//  Main_Form.Plast_ToolBar.Visible:=True;  my comment
...
{my insertochka}
 for j:=1 to Main_Form.Plast_Graph_Enter_ToolBar.ButtonCount-1 do
     Main_Form.Plast_Graph_Enter_ToolBar.Buttons[j].down:=false;
{end of my insertochka}
 if Main_Form.PlastGraphButton.Down then
   begin
    Plast_ViewGraph_Mnu.Checked := True;
    Main_Form.Plast_Graph_Enter_Panel.Visible:=True;
   end
  else
   Plast_ViewGraph_Mnu.Checked := False;
  if Main_Form.PlastKonButton.Down then
   begin
    Plast_ViewConstr_Mnu.Checked := True;
    Plast_Fd_Form.Visible:=True;
   end
  else
   Plast_ViewConstr_Mnu.Checked := False;
  if Main_Form.PlastNumberButton.Down=True then
   Plast_Num_Element.Checked:=True
  else
   Plast_Num_Element.Checked:=False;
  if (Main_Form.Plast_Sn_Cbx.ItemIndex = -1)or( Main_Form.Plast_Sn_Cbx.Items.Count<>plast.kl1 ) then
  begin
  Main_Form.Plast_SN_Cbx.clear;
  for i:=1 to plast.kl1 do
...
begin
  ...
  for i:=1 to plast.ky1 do
    begin
     ...
     What_Do:=0;
  // Какая кнопка нажата ?
  for i:=0 to main.Main_Form.Plast_Graph_Enter_ToolBar.ButtonCount-1 do
   begin
   if main.Main_Form.Plast_Graph_Enter_ToolBar.Buttons[i].Down = true then
    What_Do:=i;
   end;
...
//==============================================================================
  ...
  Visio_Form.num_force:=Main_Form.Plast_Sn_Cbx.ItemIndex+1;
...
begin
 ...
 { finally
   ...
   Cursor:=crDefault;
     erase(dopd);
     //  Plast_M.TPlast_Form(Main_Form.ActiveMDIChild).s_text_mnuClick(Sender); // ферма 4- неудобно тут - нет кнопки ок
...
//end of my insert
  ...
  Flag:=MessageDlg(chr(13)+'Расчет закончен. Показать результаты?' ,mtInformation,mbOkCancel,0);
...
if FileExists(FileName) then
 ...
 plast_rez_Form.Execute(FileName,'.vrp');
  if nf then begin main_form.PlastNumberButton.down:=false;  repaint; end;
end;
...
procedure TPlast_Form.Plast_ViewConstr_MnuClick(Sender: TObject);
begin
If Plast_ViewConstr_Mnu.Checked = True then
 begin
  Plast_FD_Form.Visible:=False;
  Plast_ViewConstr_Mnu.Checked := False;
  Main_Form.PlastKonButton.Down := False;
 end
else
 begin
 Plast_FD_Form.Visible:=True;
 Plast_ViewConstr_Mnu.Checked := True;
 Main_Form.PlastKonButton.Down := True;
 end;
end;

procedure TPlast_Form.Plast_ViewGraph_MnuClick(Sender: TObject);
begin
If Plast_ViewGraph_Mnu.Checked = True then
 begin
  Main_Form.Plast_Graph_Enter_Panel.Visible:=False;
  Plast_ViewGraph_Mnu.Checked := False;
  Main_Form.PlastGraphButton.Down := False;
...
procedure TPlast_Form.Plast_Num_ElementClick(Sender: TObject);
var
 i:integer;
begin

  if Plast_Num_Element.Checked=True then
   begin
    Main_Form.PlastNumberButton.Down:=False;
    Plast_Num_Element.Checked:=False;
    for i := Main_Form.MDIChildCount-1 downto 0 do
