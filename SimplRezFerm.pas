...
var
  ...
  nsn:integer;
  tok_name:string;
  project_changed,lastwaswithouttok,alternative_numbers:boolean;
  implementation
...
var
  ...
  if fileexists(changefileext(projectname,'.out')) then begin s_tok:=changefileext(projectname,'.out');error:=false;end
    ...
    if Main_Form.SimpleResult_TB.ImageIndex=10 then   //Imageindex = 10  -с запросом, =8 - ,без запроса
      begin
        error:=False;
...
// Напряжения
   begin
    ...
    with Param_Grd do
      begin
        RowCount:=nst+1;
{    zag1.Caption:='Напряжения';
    zag1.left:=180;
    zag2.Caption:='Коэфф.запаса прочности';
    zag2.left:=304;
    zag3.caption:='Необх.площади сечения';
    zag3.left:=470;
    zag4.caption:='Необх.моменты инерции';
    zag4.left:=636;
    zag5.Caption:='Заданные площади';
    zag5.left:=36;  }
        ...
        for i:=1 to nst do
          begin
			...
			begin
				//if(alternative_numbers = true) then
				//	Cells[j+1,i]:=formatFloat('0.0####',ps[i,j])
				//else
           ...
           for j:=nsn+1 to 3 do
            ...
            if (ps[i,j])=0 then Cells[4+j,i]:='#######'
				...
				begin
					//if(alternative_numbers = true) then
					//	Cells[4+j,i]:=formatFloat('0.0####',Abs(sd/ps[i,j]))
					//else
						Cells[4+j,i]:=formatFloat('0.00e+00',Abs(sd/ps[i,j]));
				end;
			end;
		...
		for i:=1 to nst do
			...
			begin
				  if (Sgim_cells[i,j]<>-1) then
					begin
						//if(alternative_numbers = true) then
						//	Cells[Sgim_cells[i,j]+4,i]:=formatFloat('0.0###',Abs(sd_sg/ps[i,j]))
						//else
...
zag3.Caption:='Потребные площади ['+Ferma_M.TFerma_Form(Main_Form.ActiveMDIChild).Ferm.s_lin+'2]';
...
begin
     ...
     Param_Grd.ColWidths[0] := lTmp.Width;
     ...
     for c := Param_Grd.FixedCols to Param_Grd.ColCount - 1 do begin
         case c of
1      : begin
         ...
         w   := w+Param_Grd.ColWidths[c]+2;
{
     ...
     gbDop.Top     := self.ClientHeight - gbDop.Height - ll;
...
6      : begin
     ...
     //for i := 0 to Param_Grd.ColCount - 1 do
     // AutoSizeCol(Param_Grd);
...
procedure TSimpleFermResult_Form.AutoSizeCol(Grid: TStringGrid);
const 
  ColWidthMin = 35;
var 
...
begin
  //if(alternative_numbers = true) then alternative_numbers:=false
  //else alternative_numbers:=true;
